...
object CoverallsPlugin extends Plugin with AbstractCoverallsPlugin {
  ...
  override lazy val settings = Seq(
    encoding := "UTF-8",
    coverallsTask <<= (state, baseDirectory, crossTarget, encoding) map coverallsCommand
  )
}
...
trait AbstractCoverallsPlugin  {
  ...
  def coverallsCommand(state: State, baseDirectory: File, crossTarget: File, encoding: String) = {
    ...
    if(travisJobIdent.isEmpty && userRepoToken.isEmpty) {
      ...
      sourceFiles.foreach(sourceFile => {
        val sourceReport = reader.reportForSource(baseDirectory.getAbsoluteFile + "/", sourceFile)
